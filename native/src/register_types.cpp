...
void initialize_formula90s_module(ModuleInitializationLevel level) {
    ...
    GDREGISTER_CLASS(ResetManager); GDREGISTER_CLASS(CarPhysicsConfig); GDREGISTER_CLASS(AutomaticTransmission);
    GDREGISTER_CLASS(ArcadeCarController); GDREGISTER_CLASS(ArcadeChaseCamera); GDREGISTER_CLASS(DirectionalVehicleSprite);
    GDREGISTER_CLASS(EngineAudioController); GDREGISTER_CLASS(MainMenuController); GDREGISTER_CLASS(DebugHudController);
}
