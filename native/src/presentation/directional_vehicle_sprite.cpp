...
void DirectionalVehicleSprite::_bind_methods(){
 ...
 ClassDB::bind_method(D_METHOD("get_selected_frame"),&DirectionalVehicleSprite::get_selected_frame);ClassDB::bind_method(D_METHOD("get_candidate_frame"),&DirectionalVehicleSprite::get_candidate_frame);ClassDB::bind_method(D_METHOD("get_current_relative_angle"),&DirectionalVehicleSprite::get_current_relative_angle);ClassDB::bind_method(D_METHOD("is_hysteresis_held"),&DirectionalVehicleSprite::is_hysteresis_held);
...
void DirectionalVehicleSprite::_process(double){
 if(!metadata_loaded||!is_inside_tree())return;Camera3D*camera=get_viewport()->get_camera_3d();Node3D*visual_owner=Object::cast_to<Node3D>(get_parent());ArcadeCarController*car=Object::cast_to<ArcadeCarController>(visual_owner);if(!camera||!visual_owner)return;
