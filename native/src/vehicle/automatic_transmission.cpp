...
void AutomaticTransmission::shift_up() { int next=formula90s::physics::manual_shift_up(gear,6); if(next!=gear){gear=next;shift_timer=0;emit_signal("gear_changed",gear);} }
...
double AutomaticTransmission::torque_factor(const Ref<CarPhysicsConfig> &c) const {
    if (c.is_null()) { double x = rpm / 9000.0; return Math::clamp(0.55 + 1.15 * x - 0.7 * x * x, 0.35, 1.0); }
    double x = rpm / c->get_max_rpm();
    return Math::clamp(c->get_torque_a() + c->get_torque_b() * x - c->get_torque_c() * x * x, 0.35, 1.0);
}
void AutomaticTransmission::update(double speed_mps, double throttle, double delta, const Ref<CarPhysicsConfig> &c) {
    ...
    shift_timer += delta;
    double ratio = gear < 0 ? c->get_reverse_ratio() : c->get_gear_ratios()[Math::clamp(gear - 1, 0, static_cast<int>(c->get_gear_ratios().size()) - 1)];
    rpm = formula90s::physics::engine_rpm(speed_mps, ratio, c->get_final_drive(), c->get_idle_rpm(), c->get_max_rpm());
    int max_gear = static_cast<int>(c->get_gear_ratios().size());
    int next = automatic_enabled ? formula90s::physics::choose_automatic_gear(gear,rpm,throttle,c->get_upshift_rpm(),c->get_downshift_rpm(),shift_timer>=c->get_minimum_shift_time(), max_gear) : gear;
