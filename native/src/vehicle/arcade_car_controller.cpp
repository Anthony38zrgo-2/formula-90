...
constexpr double TELEPORT_THRESHOLD_SQ = 9.0;
constexpr double STEER_ACTIVATION_SPEED = 0.2;
constexpr double DRIFT_ACTIVATION_THRESHOLD = 0.1;
constexpr double DRIFT_ROTATION_FACTOR = 0.002;
constexpr double FLOOR_SNAP_VELOCITY = -0.5;
}
...
void ArcadeCarController::_ready() { reset_presentation_pose(); cache_input_actions(); }
void ArcadeCarController::cache_input_actions() {
    Input *input = Input::get_singleton();
    input_ok = input->call("has_action", "accelerate").operator bool();
}
...
double ArcadeCarController::get_speed_kph() const { return get_velocity().length() * MPS_TO_KPH; }
...
void ArcadeCarController::poll_input() {
    Input *input = Input::get_singleton();
    if (!input_ok) return;
    throttle = input->get_action_strength("accelerate");
    brake = input->get_action_strength("brake");
    if (input->is_action_just_pressed("toggle_automatic"))
        transmission->set_automatic_enabled(!transmission->is_automatic_enabled());
    if (input->is_action_just_pressed("shift_up") && transmission->get_gear() > 0)
        transmission->shift_up();
    if (input->is_action_just_pressed("shift_down") && transmission->get_gear() > 0)
        transmission->shift_down();
    steering_input = input->get_axis("steer_left", "steer_right");
}

bool ArcadeCarController::handle_direction_change(double delta) {
    Vector3 velocity = get_velocity();
    Basis basis = get_global_basis();
    Vector3 forward = -basis.get_column(2);
    Vector3 right_vec = basis.get_column(0);
    double longitudinal = velocity.dot(forward);
    lateral_speed = velocity.dot(right_vec);
    ...
    if (requests_direction_change && formula90s::physics::is_stopped(horizontal_speed, config->get_stopped_speed_threshold())) {
        direction_stop_timer += delta;
    } else {
        direction_stop_timer = 0;
    }

    bool can_change = formula90s::physics::can_change_direction(horizontal_speed, direction_stop_timer, config->get_stopped_speed_threshold(), config->get_direction_change_delay());
    if (requests_reverse && can_change) { transmission->set_reverse(true); direction_stop_timer = 0; return true; }
    if (requests_forward && can_change) { transmission->set_reverse(false); direction_stop_timer = 0; return true; }
    return false;
}

void ArcadeCarController::apply_longitudinal_forces(double delta) {
    Vector3 velocity = get_velocity();
    Basis basis = get_global_basis();
    Vector3 forward = -basis.get_column(2);
    double longitudinal = velocity.dot(forward);
    ...
    Input *input = Input::get_singleton();
    double drive = transmission->torque_factor(config) * config->get_engine_force();
    double braking = (input_ok && input->is_action_pressed("strong_brake") ? config->get_strong_brake_force() : config->get_brake_force());
    ...
    double max_ms = (transmission->get_gear() < 0 ? config->get_reverse_max_speed_kph() : config->get_max_speed_kph()) / MPS_TO_KPH;
    longitudinal = formula90s::physics::clamp_speed(longitudinal, max_ms, max_ms);

    Vector3 right_vec = basis.get_column(0);
    velocity = forward * longitudinal + right_vec * lateral_speed;
    if (!is_on_floor()) velocity.y -= config->get_gravity() * delta;
    else velocity.y = FLOOR_SNAP_VELOCITY;
    set_velocity(velocity);
}

void ArcadeCarController::apply_lateral_forces(double delta) {
    Vector3 velocity = get_velocity();
    Basis basis = get_global_basis();
    Vector3 forward = -basis.get_column(2);
    Vector3 right_vec = basis.get_column(0);
    ...
    if (Math::abs(longitudinal) > STEER_ACTIVATION_SPEED)
        rotate_y(-steering_input * steer_rate * delta * (longitudinal >= 0 ? 1.0 : -1.0));

    Input *input = Input::get_singleton();
    double drift_input = input_ok ? input->get_action_strength("strong_brake") : 0.0;
    ...
    if (Math::abs(lateral_speed) > DRIFT_ACTIVATION_THRESHOLD)
        rotate_y(-lateral_speed * DRIFT_ROTATION_FACTOR * config->get_drift_factor());

    basis = get_global_basis();
    forward = -basis.get_column(2);
    right_vec = basis.get_column(0);
    velocity = forward * longitudinal + right_vec * lateral_speed;
    if (!is_on_floor()) velocity.y -= config->get_gravity() * delta;
    else velocity.y = FLOOR_SNAP_VELOCITY;
    set_velocity(velocity);
...
void ArcadeCarController::finalize_physics(double delta) {
    bool changed = handle_direction_change(delta);
    if (!changed) apply_longitudinal_forces(delta);
    apply_lateral_forces(delta);
    move_and_slide();
    ...
    else world_acceleration = Vector3();
    previous_world_velocity = current_world_velocity;
    acceleration_initialized = true;
    transmission->update(lateral_speed, throttle, delta, config);
    if (!is_on_floor()) state = "AIRBORNE";
    else if (transmission->get_gear() < 0) state = "REVERSE";
    else state = "DRIVING";
}
...
void ArcadeCarController::_physics_process(double delta) {
    ...
    if (!presentation_initialized || (physics_start.origin - current_physics_transform.origin).length_squared() > TELEPORT_THRESHOLD_SQ)
        reset_presentation_pose();
    previous_physics_transform = current_physics_transform;
    poll_input();
    finalize_physics(delta);
}
