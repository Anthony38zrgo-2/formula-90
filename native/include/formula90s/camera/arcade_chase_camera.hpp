...
class ArcadeChaseCamera : public Node3D { GDCLASS(ArcadeChaseCamera, Node3D)
    ...
    double lateral_swing=0.55, turn_look_offset=0.35, base_fov=62.0, speed_fov_gain=4.0, heading_smoothing=5.0, maximum_follow_lag=1.5;
    Vector3 smoothed_velocity_lead, smoothed_inertia, smoothed_look_target, filtered_acceleration, previous_inertia_source, smoothed_forward; bool initialized=false, inertia_initialized=false;
