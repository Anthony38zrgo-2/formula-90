...
def build_material_library(texture_dir: str|Path) -> dict[str,bpy.types.Material]:
    ...
    materials={
        ...
        "guardrail":texture_material("F90_Guardrail",root/manifest["shared"]["guardrail"],roughness=0.48,metallic=0.72),
        ...
        "roof":flat_material("F90_SimpleRoof",(0.34,0.33,0.31),roughness=0.94),
    }
    for asset_id,rel in manifest["assets"].items():
        alpha=("_tree_" in asset_id or "_bush_" in asset_id or "_grass_" in asset_id)
        materials[f"asset:{asset_id}"]=texture_material(f"F90_{asset_id}",root/rel,roughness=0.96 if alpha else 0.90,alpha=alpha)
