...
def main() -> int:
    ...
    expected_vegetation = {"bushes": 110, "grass": 932, "trees": 130}
    ...
    if near_grass < 678:
        failures.append(f"near-track grass is below 3x baseline: {near_grass}/678")
    if failures:
        for failure in failures:
            print(f"FAIL {failure}")
        return 2
    print(f"PASS semantic layout objects={len(stored['objects'])} vegetation={len(stored['vegetation'])}")
    print(f"PASS vegetation v2 bushes=110 grass=932 near_grass={near_grass} trees=130")
