...
from vegetation_texture_common import MAGENTA_RGB, border_connected, magenta_key_mask, transparent_edge


POSTPROCESS_ID = "vegetation_magenta_key_align_bottom"
...
def recut_image(path: Path, pass_index: int) -> dict:
    ...
    candidate = magenta_key_mask(rgba[..., :3], pass_index)
    radius = 2 if pass_index == 1 else 5
    border_candidates = border_connected(candidate)
    removed_magenta_px = 0
    # Removing a key pixel can expose another key pixel on the next edge.
    # A fixed small bound keeps this second pass deterministic and convergent.
    for _ in range(8):
        near_transparent = transparent_edge(alpha, radius)
        removable = candidate & (alpha > 0) & (near_transparent | border_candidates)
        if not removable.any():
            break
        alpha[removable] = 0
        removed_magenta_px += int(removable.sum())

    # Keep the magenta rule in RGB even when the pixel is transparent.
    rgba[..., 3] = alpha
    rgba[alpha == 0, :3] = MAGENTA_RGB
    ...
    shift_down = height - bbox[3]
    if shift_down > 0:
        shifted = np.zeros_like(rgba)
        shifted[..., :3] = MAGENTA_RGB
    ...
    return {
        ...
        "removed_magenta_px": removed_magenta_px,
        "sha256": digest,
    }
...
def update_forge_manifest(root: Path, metrics: list[dict], pass_index: int) -> None:
    ...
    recipe = {
        ...
        "key_tolerance_rgb": 80 if pass_index == 1 else 125,
        "key_min_rb": 135 if pass_index == 1 else 100,
        "key_green_gap_min": 45 if pass_index == 1 else 30,
        "key_channel_spread_max": 75 if pass_index == 1 else 100,
        "edge_radius_px": 2 if pass_index == 1 else 5,
        "bottom_anchor": "last_visible_alpha_row",
    }
    manifest["vegetation_postprocess"] = recipe
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Deterministically recut curated vegetation cards.")
    ...
    paths = sorted(
        p for p in root.glob("biomes/**/*.png")
        if category_for(p) in VEGETATION_CATEGORIES
    )
    if not paths:
    ...
    print(f"[vegetation] recut={len(metrics)} pass={ns.pass_index} root={root}")
