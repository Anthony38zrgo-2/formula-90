extends Node3D
class_name VehicleRustInputController
...
func _ready() -> void:
	if vehicle_node == null:
		push_error("[VehicleRustInputController] vehicle_node is null on '%s'" % get_path())
		return
	...
	for method in required_methods:
		if not vehicle_node.has_method(method):
			push_error("[VehicleRustInputController] vehicle_node '%s' is missing required method '%s'" % [vehicle_node.name, method])
