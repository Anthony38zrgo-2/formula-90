...
var current_cof := 2.0
var current_rolling_resistance := 1.0
var current_lateral_grip_assist := 0.05
var current_longitudinal_grip_ratio := 0.5
var current_tire_stiffness := 41000000.0
...
func initialize() -> void:
	...
	var st := surface_type if surface_type in coefficient_of_friction else "Road"
	current_cof = coefficient_of_friction.get(st, 2.0)
	current_rolling_resistance = rolling_resistance.get(st, 1.0)
	current_lateral_grip_assist = lateral_grip_assist.get(st, 0.05)
	current_longitudinal_grip_ratio = longitudinal_grip_ratio.get(st, 0.5)
	current_tire_stiffness = 1000000.0 + 8000000.0 * tire_stiffnesses.get(st, 5.0)
...
func process_forces(opposite_compression : float, braking : bool, delta : float) -> float:
	...
	if is_colliding():
		...
		var new_st := "Road"
		if surface_groups.size() > 0 and String(surface_groups[0]) in coefficient_of_friction:
			new_st = String(surface_groups[0])
		if surface_type != new_st:
			surface_type = new_st
			current_cof = coefficient_of_friction.get(surface_type, 2.0)
			current_rolling_resistance = rolling_resistance.get(surface_type, 1.0)
			current_lateral_grip_assist = lateral_grip_assist.get(surface_type, 0.05)
			current_longitudinal_grip_ratio = longitudinal_grip_ratio.get(surface_type, 0.5)
			current_tire_stiffness = 1000000.0 + 8000000.0 * tire_stiffnesses.get(surface_type, 5.0)
	else:
		last_collider = null
