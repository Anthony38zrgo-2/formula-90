...
func _run() -> void:
	...
	assert(engine_cfg_path.contains("f1_2030_v10_geometric.json"), "engine_config debe apuntar al perfil activo f1_2030_v10 geometrico")
	assert(float(chassis.get("vehicle_mass", 0.0)) == 600.0 and vehicle.mass == 600.0, "Masa del perfil f1_2030_v10 geometrico debe ser 600 kg")
