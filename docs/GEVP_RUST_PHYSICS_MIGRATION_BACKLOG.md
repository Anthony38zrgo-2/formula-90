...
| `PHY-009` | P1 | `PHY-006` | `DONE` | **Adaptador GDExtension / Godot Binding para F1-94**: C-ABI FFI (`ffi.rs`) y DLL `vehicle_physics_engine.dll` compilada (debug & release) con `f1_94_rust_vehicle.gd` (rig de 12 raycasts, duck-typing de audio y telemetría) y escena `res://scenes/vehicles/f1_94/f1_94_rust.tscn` validada en Godot headless (`test_f1_94_rust_physics.gd`). |
| `PHY-010` | P2 | `PHY-008, PHY-009` | `DONE` | **Validación de Paridad y Congelación de F1-94**: Pruebas en pista (La Chutana), verificación de lanzamiento en recta principal ($0 \to 93\text{ km/h}$, $62\text{ m}$ en $5.0\text{ s}$ con $X=0.00\text{ m}$ desviación lateral cero), exportación de telemetría de 25 columnas y certificación superada (`test_f1_94_la_chutana_parity.gd`). |
